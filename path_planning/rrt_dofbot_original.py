...
model = mujoco.Physics.from_xml_path('../robot_arms/dofbot/dofbot.xml')
...
def check_collision_with_dm_control(model, joint_config):
    ...
    """
    model.data.qpos[:5] = joint_config  # Set joint positions
    model.forward()  # Update the simulation state

    # Check for collisions
    contacts = model.data.ncon  # Number of contacts (collisions)
    return contacts == 5  # True if no contacts (collision-free)
...
def apply_rrt_path_to_dm_control(model, path, video_name="rrt_robot_motion.mp4"):
    ...
    for q in path:
        ...
        model.data.ctrl[:5] = q  # Set joint angles
...
# Example usage:
start = [-1.0, -1.0, -0.5, 0, -0.5]  # Start joint angles
goal = [0, 0.5, -0.5, -1.0, 0.5]  # Goal joint angles
joint_limits = [(-3, 3)] * 5  # Example joint limits
...
if rrt_path:
    print("Path found!")
    apply_rrt_path_to_dm_control(model, rrt_path, video_name="rrt_dofbot_motion_ori.mp4")
else:
    print("No path found!")